...
function requireInternalAuth(req, res) {
  ...
  if (!secret) {
    // Not configured: allow in dev, FAIL CLOSED in production - these
    // endpoints can send SMS and burn Claude tokens.
    if (process.env.NODE_ENV === 'production' || process.env.VERCEL_ENV === 'production') {
      console.error('INTERNAL_API_SECRET not set in production - refusing request');
      res.status(503).json({ error: 'Service not configured' });
      return false;
    }
    console.warn('INTERNAL_API_SECRET not set - internal endpoint is unprotected (dev)');
    return true;
  }
