...
const PATTERNS = {
  ...
  // Schedule listing
  list_schedule: /^list$|\b(what.*(scheduled?|coming\s+up|planned|queued)|show.*schedule|upcoming\s+posts?|my\s+posts?|schedule\s+list)\b/i,

  // Check Google reviews
  check_reviews: /^(check|any|got)\s+(my\s+|new\s+)?reviews?\b|^(any\s+)?new\s+reviews?\??$|^reviews?$/i,

  // Website audit
  website_audit: /\b(audit|check)\s+(my\s+)?(web\s?site|site|webpage|homepage)\b|^website\s+audit\b|^audit(\s+\S+\.\S+)?\s*$/i,
  ...
  // Emoji level: "no emojis", "light emojis", "lots of emojis", "use more emojis", "emoji none"
  set_emoji: /^(?:emojis?\b|(?:no|none|light|some|lots?\s*(?:of)?|more|less|fewer|use|stop|zero)\s+emojis?)/i,
  // Signature sign-off: "signature: — The Team", "sign off with X", "end posts with X"
  set_signature: /^(?:signature\b|sign[-\s]?off|end\s+(?:my\s+|every\s+|all\s+)?posts?\s+with)/i,
  // Banned words: "never say X", "don't use X", "banned words: X", "avoid the word X"
  set_banned: /^(?:never\s+(?:say|use|write)|(?:don'?t|do\s+not)\s+(?:say|use|write)|banned\s+words?|avoid\s+(?:the\s+)?(?:word|phrase|saying))\b/i,
  // Hashtags: "hashtags: #x #y", "always include #x", "default hashtags ..."
  set_hashtags: /^(?:hashtags?\b|(?:default|always\s+(?:use|include|add))\s+hashtags?|always\s+(?:include|use|add)\b.*#)/i,
  // Default CTA / link: "cta: Book now", "default link https://...", "always link to ..."
  set_cta: /^(?:cta\b|call\s*to\s*action|default\s+(?:link|cta|url)|always\s+(?:link|add\s+(?:the\s+)?link)|add\s+(?:my\s+)?(?:link|cta))/i,
  // Show current customization settings
  show_settings: /^(?:settings|my\s+settings|show\s+(?:my\s+)?settings|current\s+settings|my\s+preferences|preferences)\s*[?.!]*$/i,
};
...
function classifyIntent(text) {
  ...
  if (PATTERNS.website_audit.test(trimmed)) return INTENTS.WEBSITE_AUDIT;
  if (PATTERNS.check_reviews.test(trimmed)) return INTENTS.CHECK_REVIEWS;
  ...
  if (PATTERNS.show_settings.test(trimmed)) return INTENTS.SHOW_SETTINGS;
  if (PATTERNS.set_name.test(trimmed)) return INTENTS.SET_NAME;
  if (PATTERNS.set_voice.test(trimmed)) return INTENTS.SET_VOICE;
  if (PATTERNS.set_emoji.test(trimmed)) return INTENTS.SET_EMOJI;
  if (PATTERNS.set_signature.test(trimmed)) return INTENTS.SET_SIGNATURE;
  if (PATTERNS.set_banned.test(trimmed)) return INTENTS.SET_BANNED;
  if (PATTERNS.set_hashtags.test(trimmed)) return INTENTS.SET_HASHTAGS;
  if (PATTERNS.set_cta.test(trimmed)) return INTENTS.SET_CTA;
...
/** Parse an emoji command → 'none' | 'light' | 'lots' | '' (unclear). */
function parseEmojiLevel(text) {
  const t = (text || '').toLowerCase();
  if (/\b(none|no|zero|stop|without|remove|off|never)\b/.test(t)) return 'none';
  if (/\b(lots?|many|heavy|tons?|plenty|max(?:imum|imize)?|more)\b/.test(t)) return 'lots';
  if (/\b(light|some|few|minimal|sparing|occasional|less|fewer)\b/.test(t)) return 'light';
  return '';
}

/** Parse "signature: — The Team" / "end posts with X" → the sign-off (or ''). */
function parseSignature(text) {
  return stripCommandPrefix(
    text,
    /^(?:signature|sign[-\s]?off(?:\s+with)?|end\s+(?:my\s+|every\s+|all\s+)?posts?\s+with)/i,
  );
}

/** Parse "never say X" / "banned words: X" → the banned words/phrases (or ''). */
function parseBanned(text) {
  return stripCommandPrefix(
    text,
    /^(?:never\s+(?:say|use|write)|(?:don'?t|do\s+not)\s+(?:say|use|write)|banned\s+words?|avoid\s+(?:the\s+)?(?:word|phrase|saying))/i,
  );
}

/** Parse "hashtags: #x #y" / "always include #x" → the hashtag string (or ''). */
function parseHashtags(text) {
  return stripCommandPrefix(
    text,
    /^(?:hashtags?|(?:default|always\s+(?:use|include|add))\s+hashtags?|always\s+(?:include|use|add))/i,
  );
}

/** Parse "cta: Book now https://..." → { text, link } (either may be ''). */
function parseCta(text) {
  const raw = stripCommandPrefix(
    text,
    /^(?:cta|call\s*to\s*action|default\s+(?:link|cta|url)|always\s+(?:link(?:\s+to)?|add\s+(?:the\s+)?link)|add\s+(?:my\s+)?(?:link|cta))/i,
  );
  const urlMatch = raw.match(/https?:\/\/\S+|www\.\S+|\b[\w-]+\.(?:com|net|org|io|co|shop|store|app|biz|us)(?:\/\S*)?/i);
  const link = urlMatch ? urlMatch[0].replace(/[.,)]+$/, '') : '';
  const cta = (link ? raw.replace(link, '') : raw)
    .replace(/\s{2,}/g, ' ')
    .replace(/^[-–—:\s]+|[-–—:\s]+$/g, '')
    .trim();
  return { text: cta, link };
}
