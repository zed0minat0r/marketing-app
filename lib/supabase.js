...
async function getSocialAccounts(userId) {
  const { data, error } = await getClient()
    ...
    .eq('user_id', userId)
    .eq('is_active', true)
    .neq('platform', 'meta_user'); // internal token holder, not a publishing target
...
async function getSocialAccount(userId, platform) {
  // limit(1)+maybeSingle instead of .single(): a duplicate row (e.g. two
  // connected Pages) must degrade to "pick the most recent", never to an
  // error that reads as "not connected".
  const { data, error } = await getClient()
    .from('social_accounts')
    .select('*')
    .eq('user_id', userId)
...
async function createScheduledPost({ userId, platforms, content, mediaUrl, scheduledFor, status, topic, format }) {
  const { data, error } = await getClient()
...
/**
 * Find the most recent draft (status='draft') for a user. Used when the user
 * texts YES to approve — we flip the existing draft row to 'queued' rather
 * than creating a duplicate row from the AI's SMS reply text (which would
 * include "Reply YES to post" wrapper junk).
 */
async function getMostRecentDraftPost(userId) {
  const { data, error } = await getClient()
    .from('scheduled_posts')
    .select('*')
    .eq('user_id', userId)
    .in('status', ['draft', 'failed'])  // 'failed' so "Reply YES to try again" actually works
    .order('created_at', { ascending: false })
    .limit(1);
...
async function getAllUsersWithActiveAccounts() {
  const { data, error } = await getClient()
    .from('users')
    .select(`
      id, phone, business_name, business_type, tone, voice_notes, assistant_name, timezone, plan,
...
async function createCustomerPhoto({ userId, r2Key, publicUrl, mimeType, fileSize, caption, twilioSid, source }) {
  const { data, error } = await getClient()
...
async function updatePhotoTags(photoId, tags, status) {
  const { data, error } = await getClient()
    .from('customer_photos')
    .update({
      tags,
      tags_status: status || 'tagged',
      updated_at: new Date().toISOString(),
    })
    .eq('id', photoId)
    .select()
...
async function updatePhotoEnhancement(photoId, fields) {
...
async function listCustomerPhotos(userId, { limit = 50, includeArchived = false } = {}) {
  let q = getClient()
    .from('customer_photos')
    .select('*')
    .eq('user_id', userId)
...
async function getCustomerPhoto(photoId, userId) {
  const { data, error } = await getClient()
    .from('customer_photos')
    .select('*')
    .eq('id', photoId)
...
async function archiveCustomerPhoto(photoId, userId) {
  const { error } = await getClient()
    .from('customer_photos')
...
async function deleteCustomerPhoto(photoId, userId) {
  const { error } = await getClient()
    .from('customer_photos')
    .delete()
    .eq('id', photoId)
...
/**
 * Pick the user's best recent photo to use as fallback media for a post.
 * Prefers enhanced version when available; falls back to the original.
 * Returns null when the library is empty.
 */
async function getMostRecentPhotoForPost(userId) {
  const { data, error } = await getClient()
    .from('customer_photos')
    .select('id,public_url,enhanced_url,enhancement_status')
    .eq('user_id', userId)
  ...
  const row = (data || [])[0];
  if (!row) return null;
  const url = row.enhancement_status === 'enhanced' && row.enhanced_url
    ? row.enhanced_url
    : row.public_url;
  return url || null;
}

async function countCustomerPhotos(userId) {
