...
async function fetchComment(platform, commentId, pageToken) {
  const fields = platform === 'instagram' ? 'id,text,username,from' : 'id,message,from,permalink_url';
  ...
  if (platform === 'instagram') {
    return { id: data.id, text: data.text || '', name: data.username || data.from?.username || 'someone', fromId: data.from?.id || null };
  }
  return { id: data.id, text: data.message || '', name: data.from?.name || 'someone', fromId: data.from?.id || null };
}
