...
async function tagPhotoFromBuffer(buffer, mimeType) {
  ...
  // CLAUDE'S VISION API CANNOT READ HEIC. It accepts jpeg, png, gif and webp only, and photo-intake
  // deliberately ACCEPTS image/heic and image/heif so an iPhone photo is never rejected at the door.
  // The result was a guaranteed-to-fail API call on every native iPhone photo that Twilio did not
  // transcode: the caller swallows the error and the photo lands untagged, so nothing looks broken -
  // the photo just quietly stops matching future "use one of my photos" requests.
  //
  // Failing here instead costs nothing and says why. Converting HEIC properly needs a decoder
  // dependency in a serverless function, which is a bigger decision than this fix.
  if (media === 'image/heic' || media === 'image/heif') {
    throw new Error(`Claude vision cannot read ${media} - photo stored untagged (needs jpeg/png/gif/webp)`);
  }
