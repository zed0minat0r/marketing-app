...
function parseClaudeResponse(rawText) {
  try {
    ...
    // Log the raw response server-side only — never expose to users
    console.error('Failed to parse Claude response as JSON:', rawText.slice(0, 200));

    // Return a safe fallback — do NOT send Claude's raw output to the user
    // as it may contain internal prompt artifacts or confusing partial JSON
    return {
      reply_text: 'Something went wrong generating your response. Please try again.',
      intent: 'unknown',
      action: null,
    };
  }
...
${metrics.insights && metrics.insights.top_topic ? `- Measured winner topic: "${metrics.insights.top_topic.name}" (${metrics.insights.top_topic.avg_engagement} avg engagement over ${metrics.insights.top_topic.posts} posts)` : ''}
${metrics.insights && metrics.insights.top_format ? `- Measured winner format: ${metrics.insights.top_format.name} (${metrics.insights.top_format.avg_engagement} avg engagement)` : ''}
${metrics.experiment && metrics.experimentResult ? `- Last week's test (${metrics.experiment.description || metrics.experiment.type}): ${metrics.experimentResult.verdict === 'insufficient' ? 'not enough data yet - ' + metrics.experimentResult.detail : (metrics.experimentResult.verdict === 'variant' ? 'the test WON - ' : 'the usual approach held - ') + metrics.experimentResult.detail}` : ''}
${metrics.nextExperiment && metrics.nextExperiment.description && (!metrics.experiment || metrics.nextExperiment.started_at !== metrics.experiment.started_at) ? `- This week's new test: ${metrics.nextExperiment.description}` : ''}

Tone: ${user.tone || 'professional'}
Business type: ${user.business_type || 'business'}

Write a concise weekly SMS report (under 400 chars, plain hyphens only - never em dashes). Include:
1. Key numbers
2. Percent change vs last week (if available)
3. The measured winner and last week's test result, if provided above
4. This week's new test, if provided above - one plain sentence
