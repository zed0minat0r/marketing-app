...
async function processOnboarding(user, messageBody) {
  ...
  switch (step) {
    ...
    case 'type': {
      ...
      return {
        replyText: ONBOARDING_MESSAGES.ask_city,
        done: false,
        updatedUser: updated,
      };
    }
    ...
    case 'city': {
      // Optional - SKIP (or skip-ish replies) moves on without storing.
      const updates = { onboarding_step: 'tone' };
      const cityText = text.trim();
      if (cityText && !/^(skip|no|na|n\/a|pass)$/i.test(cityText)) {
        updates.city = cityText.slice(0, 60);
      }
