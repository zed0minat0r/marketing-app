...
/**
 * Internal module (formerly POST /api/social/post - HTTP surface removed
 * to save a Vercel function slot and shrink the public attack surface).
 *
...
const { requireInternalAuth } = require('./internal-auth');
...
async function postToInstagram(igUserId, accessToken, content, mediaUrl) {
  ...
  // Step 2: WAIT FOR THE CONTAINER TO FINISH PROCESSING. This step was missing, and it is the one
  // people miss - `scripts/post_instagram.py` in the reels project has a note about it because the
  // old Zapier route failed the same way: "Video is still processing" is not a retry problem, it is
  // publishing before Instagram has finished transcoding.
  //
  // It usually works for images because a small container is ready almost immediately, which is
  // exactly why an intermittent failure like this survives. It gets worse with bigger media, a slow
  // origin, or load - so it fails more often the more customers there are, and it would fail on
  // camera during the App Review recording.
  ...
  for (;;) {
    ...
    if (statusData.error) throw new Error(`Instagram status: ${statusData.error.message}`);

    const code = statusData.status_code;
    // Undefined means this API version did not report one - do not hang on a field that is not there.
    if (!code || code === 'FINISHED') break;
    if (code === 'ERROR' || code === 'EXPIRED') {
      throw new Error(`Instagram container ${code} before it could be published`);
    }
...
async function refreshTwitterToken(account) {
  ...
  // Update stored token
  const { upsertSocialAccount } = require('./supabase');
...
// Exported for tests: the container-status poll is the step that was missing, and a mock that
// answers FINISHED on the first call does not exercise it. See tests/instagram-container-poll.
