'use strict';

// Supported social platforms
const SUPPORTED_PLATFORMS = [
  'facebook',
  'instagram',
  'twitter',
  'linkedin',
  'pinterest',
  'google',
];

// Plan limits
const PLAN_LIMITS = {
  starter: 50,
  growth: 200,
  pro: Infinity,
};

const PLAN_NAMES = {
  starter: 'Starter ($49/mo)',
  growth: 'Growth ($99/mo)',
  pro: 'Pro ($199/mo)',
};

// Onboarding steps in order
const ONBOARDING_STEPS = ['name', 'type', 'city', 'tone', 'connect', 'photos', 'done'];

const BUSINESS_TYPES = ['restaurant', 'retail', 'service', 'ecommerce', 'other'];
const TONE_OPTIONS = ['casual', 'professional', 'bold', 'friendly'];

// Intent categories
const INTENTS = {
  ONBOARDING: 'onboarding',
  POST: 'post',
  SCHEDULE: 'schedule',
  ANALYTICS: 'analytics',
  WEBSITE_AUDIT: 'website_audit',
  CHECK_REVIEWS: 'check_reviews',
  HELP: 'help',
  CONNECT: 'connect',
  CANCEL: 'cancel',
  APPROVE: 'approve',
  EDIT: 'edit',
  SKIP: 'skip',
  LIST_SCHEDULE: 'list_schedule',
  DELETE_DATA: 'delete_data',
  REFERRAL: 'referral',
  PHOTO_INTAKE: 'photo_intake',
  PHOTO_INTAKE_PREF: 'photo_intake_pref',
  COMPLIANCE_STOP: 'compliance_stop',
  COMPLIANCE_START: 'compliance_start',
  COMPLIANCE_HELP: 'compliance_help',
  // Post-onboarding customization commands
  SET_TONE: 'set_tone',
  SET_VOICE: 'set_voice',
  SET_NAME: 'set_name',
  SET_EMOJI: 'set_emoji',
  SET_SIGNATURE: 'set_signature',
  SET_BANNED: 'set_banned',
  SET_HASHTAGS: 'set_hashtags',
  SET_CTA: 'set_cta',
  SHOW_SETTINGS: 'show_settings',
  UNKNOWN: 'unknown',
};

// Rate limit config (per phone number)
const RATE_LIMITS = {
  sms_inbound: { requests: 1, window: 1000 }, // 1 per second
  ai_generate: { requests: 10, window: 60000 }, // 10 per minute
  oauth: { requests: 5, window: 3600000 }, // 5 per hour
};

// Claude models
const CLAUDE_MODELS = {
  fast: 'claude-sonnet-4-5',
  powerful: 'claude-opus-4-5',
};

// System prompt builder
function buildSystemPrompt(user, connectedPlatforms) {
  const platforms = connectedPlatforms && connectedPlatforms.length > 0
    ? connectedPlatforms.join(', ')
    : 'none connected yet';

  const generationsLeft = user.plan === 'pro'
    ? 'unlimited'
    : `${Math.max(0, (user.generations_limit || PLAN_LIMITS[user.plan] || 50) - (user.generations_used || 0))} generations left this month`;

  // The user can name their assistant; default to the Sidekick brand.
  const assistantName = (user.assistant_name || '').trim() || 'Sidekick';
  // Free-text voice the user described takes precedence over the 4 preset tones.
  const voiceLine = (user.voice_notes || '').trim()
    ? `- Voice (their words, follow this closely): ${user.voice_notes.trim()}`
    : `- Tone: ${user.tone || 'professional'}`;

  // Brand rules the user configured — only rendered when set. These are hard
  // constraints on every drafted post.
  const brandRules = [];
  const emoji = (user.emoji_level || '').trim();
  if (emoji === 'none') brandRules.push('- Use NO emojis at all.');
  else if (emoji === 'light') brandRules.push('- Use emojis sparingly (at most one per post).');
  else if (emoji === 'lots') brandRules.push('- Use plenty of emojis — lean into them.');
  if ((user.banned_words || '').trim()) brandRules.push(`- NEVER use these words/phrases: ${user.banned_words.trim()}`);
  if ((user.hashtags || '').trim()) brandRules.push(`- Always include these hashtags: ${user.hashtags.trim()}`);
  const ctaText = (user.cta_text || '').trim();
  const ctaLink = (user.cta_link || '').trim();
  if (ctaText || ctaLink) brandRules.push(`- Work in this call-to-action${ctaLink ? ' and link' : ''}: ${[ctaText, ctaLink].filter(Boolean).join(' ')}`);
  if ((user.signature || '').trim()) brandRules.push(`- End every post with this sign-off exactly: ${user.signature.trim()}`);
  const brandRulesBlock = brandRules.length
    ? `\n\nBrand rules (hard requirements on every post):\n${brandRules.join('\n')}`
    : '';

  // Measured performance steering - only ever from stored, computed insights
  // (lib/growth.js). Absent until the weekly job has real data.
  const wins = [];
  const ci = user.content_insights || {};
  if (ci.top_topic) wins.push(`- Topic "${ci.top_topic.name}" averages ${ci.top_topic.avg_engagement} engagement across ${ci.top_topic.posts} posts - lean toward it.`);
  if (ci.top_format) wins.push(`- "${ci.top_format.name}" posts average ${ci.top_format.avg_engagement} engagement across ${ci.top_format.posts} posts - prefer that format.`);
  if (ci.best_hour_local) wins.push(`- Posts around ${ci.best_hour_local.hour}:00 local average ${ci.best_hour_local.avg_engagement} engagement - suggest that window when scheduling.`);
  if (ci.best_post && ci.best_post.engagement > 0) wins.push(`- Their best recent post (${ci.best_post.engagement} engagement): "${ci.best_post.content_preview}"`);
  const exp = user.active_experiment;
  if (exp && exp.status === 'live' && exp.description) wins.push(`- LIVE EXPERIMENT: this week we are testing ${exp.description}. When it fits naturally, draft toward the test.`);
  const measuredWinsBlock = wins.length
    ? `\n\nWhat has measurably worked for this business (from their own analytics - never invent numbers beyond these):\n${wins.join('\n')}`
    : '';

  return `You are ${assistantName}, an AI marketing assistant for small businesses. You communicate exclusively via SMS. Keep responses under 300 characters when possible (SMS segment limit is 160 chars; 2 segments max preferred). Be direct and conversational — you are texting, not writing an email.

Business context:
- Name: ${user.business_name || 'Not set'}
- Type: ${user.business_type || 'Not set'}${(user.city || '').trim() ? `\n- City: ${user.city.trim()} (use local flavor - neighborhood references, local events, geo-relevant hashtags)` : ''}
${voiceLine}
- Connected platforms: ${platforms}
- Plan: ${PLAN_NAMES[user.plan] || user.plan} (${generationsLeft})
- Timezone: ${user.timezone || 'America/New_York'}
- Current time: ${new Date().toISOString()} (UTC)

When you produce a "scheduled_for" ISO 8601 datetime, interpret the user's
natural-language time ("Friday at 5pm", "tomorrow morning") in their LOCAL
timezone above, then convert to UTC for the output. Never assume UTC. If
the user says "5pm" and timezone is America/New_York, that's 9pm/10pm UTC
(depending on DST). Output the resulting UTC instant as the ISO string.

You can:
1. Draft social media posts (user must approve before posting)
2. Schedule posts for later
3. Provide marketing tips tailored to their business type
4. Summarize engagement analytics
5. Help connect social media accounts (Facebook, Instagram, LinkedIn, Google Business, X, Pinterest)
6. Save photos the user texts in to their photo library — they're tagged and auto-enhanced

The user has a photo library built from photos they text directly to this number (MMS). When they ask for an image post, the most recent library photo is used automatically — you do NOT need to ask them for an image URL. If their library is empty and they want an image-required post (Instagram, Pinterest, Google Business), instruct them to "text me a few photos of your work or space first."

When drafting posts:
- Match the voice/tone described in the business context above${(user.voice_notes || '').trim() ? ' — mirror their described voice exactly' : ` (${user.tone || 'professional'})`}
- Include relevant hashtags
- Keep it platform-appropriate (LinkedIn = longer/professional, X = ≤280 chars, IG = caption + hashtag-friendly)
- Always end with: "Reply YES to post, EDIT to change, LATER to schedule, or SKIP to cancel."${brandRulesBlock}${measuredWinsBlock}

Never post without explicit user approval (YES response).

When user says YES/EDIT/SKIP/LATER, recognize it as a response to the last post draft.

Respond ONLY with valid JSON in this exact format — no extra text, no markdown:
{
  "reply_text": "Your SMS reply text here",
  "intent": "one of: post|schedule|analytics|help|connect|cancel|approve|edit|skip|list_schedule|unknown",
  "action": null
}

For post drafts, action should be:
{
  "type": "draft_post",
  "platform": "facebook|instagram|twitter|linkedin|pinterest|google|all",
  "content": "the post content",
  "scheduled_for": null,
  "topic": "a 2-3 word topic label for this post (e.g. 'weekend special', 'new arrival')",
  "format": "one of: photo|offer|announcement|tip|question|general"
}

For schedule actions, action should be:
{
  "type": "schedule_post",
  "platform": "facebook|instagram|twitter|linkedin|pinterest|google|all",
  "content": "the post content",
  "scheduled_for": "ISO 8601 datetime string",
  "topic": "a 2-3 word topic label for this post",
  "format": "one of: photo|offer|announcement|tip|question|general"
}`;
}

// Onboarding prompts
const ONBOARDING_MESSAGES = {
  welcome: "Welcome to Sidekick! I am your AI marketing team, right here in your texts.\n\nFirst -- what is your business called?",
  ask_type: (name) => `${name} -- great name! What kind of business?\n(restaurant, retail, service, ecommerce, other)`,
  ask_city: "What city are you in? (like 'Phoenixville, PA') -- I use it for local flavor in your posts. Reply SKIP if you'd rather not say.",
  ask_tone: "And how should your marketing sound?\n(casual, professional, bold, friendly)",

  // Step 4 — connect first platform. Default suggestion is Facebook (single
  // OAuth grants both Facebook Page and Instagram posting rights). They can
  // text another platform to pick differently, or SKIP to defer.
  ask_connect: "Next -- let's connect a social account so I can actually post for you.\n\nTap below to connect Facebook + Instagram together:\n{link}\n\nOr reply: INSTAGRAM, GOOGLE, LINKEDIN, X, PINTEREST to pick a different one. Reply SKIP to do this later.",

  // Sent after the user picks a non-default platform.
  ask_connect_alt: (label, link) => `Tap below to connect ${label}:\n${link}\n\nOr reply with a different platform name. Reply SKIP to do this later.`,

  ask_connect_invalid: "I did not recognize that platform. Try: INSTAGRAM, FACEBOOK, GOOGLE, LINKEDIN, X, PINTEREST, or SKIP.",

  // Step 5 — photo library kickoff. We ask for 3-5 but accept any (1 is enough
  // to advance) and accept DONE as an explicit move-on.
  ask_photos: "Awesome -- one last thing. Text me 3-5 photos of your space, products, or finished work -- anything you'd want to show off. I'll save them so I can use them in posts later.\n\nReply DONE when you're finished, or SKIP to do this later.",

  photo_received_during_onboarding: (count) => count === 1
    ? "Got it -- 1 photo saved. Keep them coming, or reply DONE when finished."
    : `Got it -- ${count} photos saved. Keep them coming, or reply DONE when finished.`,

  complete: (name) => `All set! ${name} is ready to roll.\n\nHere is what I can do:\n- Write social posts (just say "post about our Friday special")\n- Schedule content for any day/time\n- Auto-publish to your connected accounts\n- Track your marketing performance\n\nText me a photo anytime to add to your library. Text "Connect <platform>" to add more accounts.`,

  complete_no_connection: (name) => `All set! ${name} is ready.\n\nOne thing -- you haven't connected a social account yet, so I can draft posts but can't publish them. Text "Connect Facebook" (or Instagram / Google / LinkedIn / X / Pinterest) when you're ready.\n\nIn the meantime, text "Write a post about [topic]" and I'll draft something.`,
};

// Error messages
const ERROR_MESSAGES = {
  rate_limit: "Slow down! Wait a moment before sending another message.",
  generation_limit: (plan) => `You have used this month's included generations on the ${plan} plan. Your count resets on the 1st - approving, scheduling, and canceling posts still work anytime.`,
  general: "Something went wrong on our end. Please try again in a moment.",
  not_onboarded: "Please complete setup first. What is your business name?",
};

module.exports = {
  SUPPORTED_PLATFORMS,
  PLAN_LIMITS,
  PLAN_NAMES,
  ONBOARDING_STEPS,
  BUSINESS_TYPES,
  TONE_OPTIONS,
  INTENTS,
  RATE_LIMITS,
  CLAUDE_MODELS,
  buildSystemPrompt,
  ONBOARDING_MESSAGES,
  ERROR_MESSAGES,
};
