...
module.exports = async function handler(req, res) {
  // Vercel Cron sends GET; allow POST too for manual invocation/testing.
  ...
  try {
    ...
    // While we're here, also clean expired oauth_states + oauth_links.
    // The SQL function was defined in migration 001 but nothing called it,
    // so abandoned-OAuth rows have been accumulating forever.
    let oauthCleaned = 0;
    try {
      const { error: rpcErr } = await client.rpc('cleanup_expired_oauth');
      if (rpcErr) {
        console.warn('cleanup_expired_oauth RPC failed:', rpcErr.message);
      } else {
        oauthCleaned = 1; // we don't get a row count from RPC, just success
      }
    } catch (err) {
      console.warn('cleanup_expired_oauth threw:', err.message);
    }
