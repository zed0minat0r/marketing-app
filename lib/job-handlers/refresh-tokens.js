...
async function refreshMetaToken(account) {
  ...
  const url = `https://graph.facebook.com/v22.0/oauth/access_token?grant_type=fb_exchange_token&client_id=${appId}&client_secret=${appSecret}&fb_exchange_token=${account.access_token}`;
...
module.exports = async function handler(req, res) {
  ...
  // ── PLANNER / WORKER, same reason as weekly-summary. ─────────────────────────────────────────
  // This loaded every account due for refresh and walked them in one 300s function. It is less
  // exposed than the weekly summary because it only selects accounts expiring inside 15 days, not
  // everyone - but the failure is worse when it does happen: a token that does not get refreshed
  // gets its account DEACTIVATED, so that customer silently stops publishing. Timing out here loses
  // exactly the accounts at the end of the list, every day, until someone notices.
  const BATCH_SIZE = 25;
  ...
  if (!accountIds) {
    ...
    for (let from = 0; ; from += PAGE_SIZE) {
      ...
      if (pageErr) {
        console.error('Token refresh: failed to page accounts:', pageErr);
    ...
    for (const batch of batches) {
      try {
        ...
        await publishJob('refresh-tokens', { accountIds: batch });
        enqueued += 1;
      } catch (err) {
        console.error('Token refresh: could not enqueue a batch, will run inline:', err.message);
        inline.push(...batch);
      }
    }
    if (!inline.length) {
      return res.status(200).json({ planned: ids.length, batches: batches.length, enqueued });
    }
    // Fall through and process whatever could not be enqueued, rather than skipping a refresh.
    req.body = { ...(req.body || {}), accountIds: inline };
  ...
  for (const account of decryptedAccounts) {
    ...
    try {
      if (account.platform === 'facebook' || account.platform === 'instagram' || account.platform === 'meta_user') {
