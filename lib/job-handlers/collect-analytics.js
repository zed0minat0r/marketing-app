...
async function collectFacebookInsights(postId, fbPostId, pageToken) {
  ...
  const url = `https://graph.facebook.com/v22.0/${fbPostId}/insights?metric=${metrics.join(',')}&access_token=${pageToken}`;
  ...
  // Get comments count
  const commentsRes = await fetch(`https://graph.facebook.com/v22.0/${fbPostId}?fields=comments.summary(true)&access_token=${pageToken}`);
  const commentsData = await commentsRes.json();
  results.comments = commentsData.comments?.summary?.total_count || 0;

  // Get shares count
  const sharesRes = await fetch(`https://graph.facebook.com/v22.0/${fbPostId}?fields=shares&access_token=${pageToken}`);
...
async function collectInstagramInsights(postId, igMediaId, accessToken) {
  // v22+: the 'impressions' media metric was removed; 'views' is its successor.
  const url = `https://graph.facebook.com/v22.0/${igMediaId}/insights?metric=reach,views,likes,comments,shares,saved&access_token=${accessToken}`;
  ...
  for (const item of (data.data || [])) {
    switch (item.name) {
      ...
      case 'views': results.impressions = item.values?.[0]?.value || 0; break; // stored in the impressions column as its successor
      ...
      for (const platform of post.platforms) {
        ...
        try {
          ...
          if (platform === 'facebook') {
            ...
            // Prefer the stored media id (written at publish time since the
            // permalink fix); fall back to parsing legacy /p/<id> URLs.
            let igMediaId = post.published_urls?.instagram_media_id || null;
            if (!igMediaId) {
              try {
                const url = new URL(platformPostUrl);
                const parts = url.pathname.split('/').filter(Boolean);
