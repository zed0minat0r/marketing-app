...
const { getScheduledPost, getClient, logMessage } = require('../supabase');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (user) {
      ...
      // LOG IT. Publish result texts were never written to the conversation, and that is not just a
      // hole in the AI's memory of what it told the customer - the inbound handler decides what a
      // bare "YES" means by looking at the intent of the LAST outbound message. With these missing,
      // the last outbound could still be an unanswered comment-reply draft, and a YES meant for
      // "retry my post" would post that comment reply instead.
      await logMessage({
        userId: user.id, direction: 'outbound', body: msg, intent: 'publish_result',
      }).catch(console.error);
    }
    ...
    if (user) {
      const failMsg = `Your post couldn't be published after ${attemptsSoFar} attempts. Reply YES to try again, or EDIT to change it. (${String(err.message).slice(0, 120)})`;
      await sendSms(user.phone, failMsg).catch(console.error);
      // This one matters most: it explicitly invites a YES, so the last-outbound intent has to say
      // that a YES here means "retry the post" rather than leaving a stale draft to claim it.
      await logMessage({
        userId: user.id, direction: 'outbound', body: failMsg, intent: 'publish_failed',
      }).catch(console.error);
    }
