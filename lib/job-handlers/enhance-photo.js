...
async function verifyQStashSignature(req) {
  ...
  if (!currentKey) {
    // FAIL CLOSED in production - unsigned triggers must never spend Replicate credits.
    if (process.env.NODE_ENV === 'production' || process.env.VERCEL_ENV === 'production') {
      console.error('QStash signing key not set in production - refusing enhance request');
      return false;
    }
    return true; // dev/test path
  }
