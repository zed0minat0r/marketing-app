...
async function maybeAlert(key, subject, body, { sms = false } = {}) {
  ...
  // Same fallback as the waitlist notifier - alerts must not depend on an
  // optional env var being remembered.
  const to = process.env.NOTIFY_EMAIL || 'mmodica3@gmail.com';
  let emailed = false;
  if (to) {
    try {
