...
function buildPublishUrl() {
  const appUrl = (process.env.APP_URL || 'https://marketing-app-navy.vercel.app').replace(/\/$/, '');
...
/**
 * Enqueue any job action as its own invocation (used by the daily dispatch
 * so one slow job can't starve the others inside a single function budget).
 */
async function publishJob(action, body = {}) {
  const appUrl = (process.env.APP_URL || 'https://marketing-app-navy.vercel.app').replace(/\/$/, '');
  // QStash replays these headers on delivery; the cron Bearer lets the
  // sub-handler's requireCronAuth pass (deliveries don't carry it otherwise).
  const headers = process.env.CRON_SECRET
    ? { Authorization: `Bearer ${process.env.CRON_SECRET}` }
    : undefined;
  const res = await getClient().publishJSON({
    url: `${appUrl}/api/jobs/${action}`,
    body,
    headers,
  });
  return { messageId: res.messageId };
